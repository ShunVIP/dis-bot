...
- вкладки `Обзор`, `Управление`, `Настройки`, `KGTD.env`, `Лог`, `Сценарии`;
- сохранение локальных параметров (`Tailscale IP`, токен bridge, VPS host/user, SSH key, URL панели);
- быстрые индикаторы `Bridge`, `VPS`, `Tailscale`;
- установка локальных зависимостей;
- запуск бота локально в отдельном окне;
- запуск/остановка bridge;
- связка VPS с ПК;
- запуск полного bridge-комплекта;
- отключение тяжёлой модели на VPS;
...
- просмотр статуса VPS;
- открытие веб-панели;
- просмотр сценариев “что делать по шагам”.
...
1. В GUI нажать `Скачать messages.db с VPS`.
2. В GUI нажать `Открыть обучение моделей`.
3. В окне обучения выбрать:
...
1. В GUI нажать `Запустить bridge`.
2. Потом нажать `Связать VPS с ПК`.
3. Если больше не нужен:
   - `Отключить тяжёлые модели на VPS`;
   - `Остановить bridge`.
...
4. снова открыть окно обучения из GUI
...
3. в GUI:
   - `Запустить bridge` чтобы включить;
   - `Отключить тяжёлые модели на VPS` чтобы разорвать связь;
   - `Остановить bridge` чтобы погасить локальный bridge.
