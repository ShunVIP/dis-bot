...
### Локальный GPT-цикл с ПК

Если основная база сообщений живёт на VPS, а GPT ты обучаешь на ПК, рабочий цикл такой:
- запустить `train_models.bat`
- при старте выбрать синхронизацию `messages.db` с VPS
- обучить нужного пользователя или всех локально на ПК
- не отправлять тяжёлые GPT-модели на VPS
