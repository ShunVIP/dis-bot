...
<body>
  <div class="wrap">
    ...
    <div class="card">
      ...
      <p class="help">По умолчанию серверу запрещено GPT-дообучение и полная профилактика. Это защищает VPS от перегруза и случайного запуска тяжёлых задач из Discord.</p>
    </div>

    <div class="card">
      <h2>Быстрый переключатель</h2>
      <form method="post" action="/remote-models?token={escape(WEB_ADMIN_TOKEN)}">
...
async def _index(request: web.Request) -> web.Response:
    if not _is_authorized(request):
        return web.Response(
            text="Admin token required. Open /?token=YOUR_WEB_ADMIN_TOKEN",
            status=401,
