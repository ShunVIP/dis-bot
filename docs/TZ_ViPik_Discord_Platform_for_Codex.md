...
> Примечание 2026-07-13: разделы про GPT/Persona/model bridge устарели. Текущее обязательное решение описано в README: пародии только Markov, ML развивается вне пародий.
...
- безопасную работу с VPS, env-секретами, базами и переносимыми лёгкими ML-артефактами.
...
- Markov-пародии и обучение;
- локальные модели в `models/`;
- VPS deploy scripts;
- systemd templates;
- веб-админку в `core/admin_panel.py`;
- версионированный manifest лёгких ML-артефактов;
...
- Не переносить тяжёлые модели на VPS; использовать только проверенные лёгкие артефакты.
